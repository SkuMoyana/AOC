namespace Day5Part2
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> invalidUpdates = new();
            ...
            foreach (string update in invalidUpdates)
            {
                ...
                for (int i = 1; i < pages.Length + 1; i++)
                {
                    ...
                    var applicableRules = rules.Where(x => x.Contains(pages[i].ToString())).Select(x => x.Split("|")).ToList();
